...
export default function VehicleContextMenu({
  ...
  return (
    <>
      {/* ========== PC: 드롭다운 (lg 이상) - absolute 기반 ========== */}
      <div
        ref={menuRef}
        className="absolute z-50 hidden min-w-[180px] overflow-hidden rounded-xl border border-gray-200 bg-white py-1.5 shadow-xl lg:block dark:border-[#454c5c] dark:bg-[#262a33] dark:shadow-black/40"
      ...
      <AnimatePresence>
        {contextMenu.show && (
          <motion.div
            ...
            className="absolute inset-x-0 bottom-0 z-50 rounded-t-2xl bg-white shadow-2xl ring-1 ring-black/5 lg:hidden dark:bg-[#1c1f26] dark:ring-white/10"
            onClick={(e) => e.stopPropagation()}
          >
            {/* 드래그 핸들 */}
